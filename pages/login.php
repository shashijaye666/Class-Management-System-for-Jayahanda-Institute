...
function insertuser($request)
{
    $user = new user();
    // $Pwdhash = hash('md5',$request['PassWord']);
    $id = $user->insertUser($request["name"],$request["username"],hash('md5',$request['password']),$request["userlevel"]);
    if($id > 0)
       echo json_encode(1);
    else
       echo json_decode(0);
 ...
 function update($request){
    ...
    $us = new user();
    $id =$us->edit($request['name'],$request['username'],$request['password'],$request['userlevel'],$request['id']);
    if($id > 0)
    echo json_encode(1);
 else
    echo json_decode(0);
